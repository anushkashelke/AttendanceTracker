...
class FaceDetectionThroughTflite {
  static final FaceDetectionThroughTflite _tfliteService =
      FaceDetectionThroughTflite._internal();
  factory FaceDetectionThroughTflite() {
  ...
  setCurrentPrediction(CameraImage cameraImage, Face face) {
    ...
    input = input.reshape([4, 112, 112, 3]);
  ...
  Float32List imageToByteListFloat32(imglib.Image image) {
    /// input size = 112
    var convertedBytes = Float32List(4 * 112 * 112 * 3);
  ...
  String _searchResult(List predictedData) {
    ...
    /// if no faces saved
    if (data.length == 0) return null as String;
    double minDist = 999;
    double currDist = 0.0;
    String predRes = null as String;
